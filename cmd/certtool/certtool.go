...
var (
	...
	spec       = flag.String("spec", "", "Path to a YAML file specifying certificates to generate.")
	outputDir  = flag.String("output-dir", ".", "Output directory for certificates generated by --chain or --spec.")
	exportSpec = flag.String("export-spec", "", "Export the generated certificate spec as a YAML file instead of generating certificates.")
...
func certtoolMain() int {
	...
	if *exportSpec != "" {
		return writeSpec(nodes)
	}

	results, err := certtool.GenerateHierarchy(nodes, dir, parentKP)
	if err != nil {
		zap.S().Error(err)
		return 1
	}
...
func buildSpec() ([]certtool.HierarchyNode, *certtool.KeyPair, string, error) {
	...
	case *spec != "":
		nodes, err = certtool.ParseHierarchyFile(*spec)
		if err != nil {
			return nil, nil, "", err
		}
	default:
		if *parentPublicCertificate != "" {
			parentKP, err = certtool.ReadKeyPairFromFile(*parentPublicCertificate, *parentPrivateKey)
			if err != nil {
				return nil, nil, "", err
			}
		}
	...
	nodes = certtool.ApplyDefaults(nodes, defaults)
	return nodes, parentKP, dir, nil
}

func buildDefaults() (*certtool.Args, error) {
	if *validity <= 0 {
...
func writeSpec(nodes []certtool.HierarchyNode) int {
	data, err := certtool.MarshalSpec(nodes)
	if err != nil {
		zap.S().Error(err)
		return 1
	}
