...
var (
	...
	chain      = flag.String("chain", "", "Generate a certificate hierarchy. A single number is the chain depth (e.g. 5 = 5-layer linear chain). Comma-separated numbers set the breadth per layer (e.g. 1,2,3 = 1 root, 2 intermediates per root, 3 leaves per intermediate).")
...
func runChainMode() int {
	...
	specs, err := certtool.ChainToSpec(widths, *commonName, *organization)
	if err != nil {
		zap.S().Error(err)
		return 1
	}
...
func writeSpecFile(specs []certtool.CertificateSpec) int {
	...
	zap.S().Infof("Spec written to %s", *exportSpec)
	return 0
}
